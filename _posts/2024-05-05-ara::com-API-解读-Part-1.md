...
注意：一个Proxy类的实例只能与一个服务实例进行通信；如果一个Proxy类需要同时与多个服务实例通信，则必须实例化多个Proxy类的实例；Proxy类的FindService方法是`static`的。
...
ComAPI仅仅提供了Proxy/Skeleton类的标准方法接口，不提供类的封装以及更上层SWC的封装方法；AUTOSAR文档是采用继承的方式在Proxy/Skeleton类中实现相关Service的实现，例如event handler，method handler等。实际实现中，也可以采用set handler的方式。
...
Instance identifier和Instance specifier这两个概念必须结合AUTOSAR的模型来理解，否则很难get到其中的概念。
