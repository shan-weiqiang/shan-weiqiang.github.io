---
layout: post
title:  "Type Erasure: Part I — Core Logic Behind Type Erasure"
...
- Different implementation for different types might be manually coded or generated by compiler(C++)
  ...
  - Virtual inheritence
- Binding process happens during compile time/construction time 
  - for template, compiler generate different functions with the same signature 
  - for virtual methods, compiler generate different methods for different classes with the same signature 
  - the same applies to manually written functions 
  - all above happens during compile time, defined by how codes are written 
  - the binding is finished after compiling,runtime dispatch happens between these functions with same signature 
