...
```mermaid
flowchart LR
  msgFile["DemoStatus.msg"] --> adapter["rosidl_adapter → DemoStatus.idl"]
  adapter --> generators["rosidl_generate_interfaces"]
  generators --> langBind["rosidl_generator_c / _cpp / _py"]
  generators --> implLayers["fastrtps_* / introspection_*"]
  generators --> dispatchers["rosidl_typesupport_c / _cpp"]
```
...
sequenceDiagram
  ...
  participant Entry as dispatcher_entry_symbol
  participant Disp as runtime_func_resolver
  participant Lib as libdemo_pkg__rosidl_typesupport_fastrtps_c.so
  participant Impl as fastrtps_entry_symbol
  participant CB as __callbacks_DemoStatus

  User->>Entry: Stage 0 link import or dlsym
  Entry-->>User: dispatcher handle
  User->>Disp: handle->func(handle, "rosidl_typesupport_fastrtps_c")
  Disp->>Lib: dlopen on first use
  Disp->>Lib: dlsym symbol from map
  Lib->>Impl: call impl entry()
  Impl-->>Disp: FastDDS handle
  Disp-->>User: FastDDS handle
  User->>CB: cdr_serialize via handle->data
```
...
```mermaid
flowchart TB
  subgraph cTrack [C_track]
    py["rclpy / rcl C API"] --> tsC["rosidl_typesupport_c"]
    tsC --> fastC["rosidl_typesupport_fastrtps_c"]
    tsC --> introC["rosidl_typesupport_introspection_c"]
  end
  subgraph cppTrack [C++_track]
    rclcpp["rclcpp nodes"] --> tsCpp["rosidl_typesupport_cpp"]
    tsCpp --> fastCpp["rosidl_typesupport_fastrtps_cpp"]
    tsCpp --> introCpp["rosidl_typesupport_introspection_cpp"]
  end
  fastC --> dds["Fast DDS CDR"]
  fastCpp --> dds
```
