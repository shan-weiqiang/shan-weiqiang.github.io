...
What if threads from two different process want to do synchronization using futex? It's intuitive to think that the futex word should be in a shared memroy region. This is indeed the solution. If the futex word is in a shared memroy region from different processes, *inter-process* synchronization can be achieved. It is rather easy from the user side, since what the user need to do is simply create a shared memroy region and put the futex word in there and share it across processes. The usage of the futex is totally the same as in non-shared memory scenario. However, the kernel now has a headache to solve: how to uniquely identify inter-process futex？

In intra-process scenario, the kernel can use PID plus the virtual address of the futex word to uniquely identify a futex inside the kernel. In inter-process scenario, this is invalid since the futex word have different virtual address in each processes. The requirement is that kernel need to find a way to uniquely identify a futex, even across different virtual address space. One possible solution is to use the physical address to uniquely identify the futex, since the physical address is the same across all processes in this situation. However, the futex might be swapped out of RAM during runtime and every kernel have it’s own page replacement algorithm. After the memroy region containing the futex is swapped in, it might have a different physical address with the previous one. At the end, for a inter-process futex to work, it’s kernel’s job to ensure:
