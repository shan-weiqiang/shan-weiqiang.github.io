...
1. Application process calls `write` to write bytes into TCP socket, kernel check whether there is available buffer space, **Acked(ready for writing from application)** in the diagram, to store those bytes. If yes, kernel copies all the bytes into this area, starting from **AppDataWrtPtr**. If now enough space, kernel use all of the **Acked(ready for writing from application)** space. In both scenario, `write` return the number of bytes copied.
