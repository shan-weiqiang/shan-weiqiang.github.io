...
本文不是关于SOME/IP SD的翻译，也不是漫谈，而是按照原文章节对各个章节中的难以理解的部分进行解读。限于个人理解，如有错误，请以原文为准。
...
SOME/IP-SD基于SOME/IP协议，且底层的传输层协议使用UDP
...
了解了SOME/IP-SD的消息结构，也就理解了SOME/IP-SD的服务发现协议。也就是理解如下各个字段的含义。可以看出SOME/IP-SD是一个普通SOME/IP消息的payload，也就是说SD是基于SOME/IP协议进行的。可以将SOME/IP-SD协议理解为基于SOME/IP协议之上的一个应用层协议。

![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled.png)
...
- Method-ID是固定值0x8100
- Client-ID是固定值0x0000：一个host上只存在一个SOME/IP-SD 实例
...
- 前8 bits成为flags，其中第一个bit为1表示Reboot；当host启动后开始发送的所有SOME/IP-SD消息都将这个bit设为1，表示host正在启动；当session ID自增超过16 bits的最大值，重新从1开始自增时，将这个bit设置为0，表示host启动完成
...
- 实际发现内容分为两类：
    ...
    ![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%201.png)
    
    > 注意这里包含Instance ID信息，但是是作为SOME/IP的payload，不是SOME/IP头部的一部分
    > 
    - EventGroup类型：除了包含服务发现类型以外，最后包含了EventGroupID信息
    
    ![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%202.png)
...
- 第一段中的options是所有SOME/IP-SD共用的options
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%203.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%204.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%205.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%206.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%207.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%208.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%209.png)
...
- 所有SD的消息，无论单播还是组播，都需要发送到SD_PORT
- 所有组播的SD消息，都需要使用SD_MULTICAST_IP
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%2010.png)

说明如下：

- 状态机进入Initial Wait Phase阶段后需要在INITIAL_DELAY_MIN和INITIAL_DELAY_MAX区间随机的等待一段时间，等待完成后直接进入下一个阶段
- 在Repetition Phase阶段，从REPETITION_BASE_DELAY间隔值开始，每发送一次，间隔时间扩大一倍，直到发送次数达到REPETITION_MAX次，进入下一个阶段
- 在Maint Phase，按照CYCLIC_ANNOUNCE_DELAY周期性的间隔发送
- 在Repetition Phase和Main Phase阶段，如果收到FindService请求，等待一个在RequestResponseMaxDelay和RequestResponseMinDelay的随机值后发送一次
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%2011.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%2012.png)

单播EventGroup server端状态机

---

![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%2013.png)

组播EventGroup server端状态机

---

![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%2014.png)
...
![Untitled](SOME%20IP%20Service%20Discovery%20Protocol%20Specification%20dfadffeca0c7489bb753bfd44d2b0eec/Untitled%2015.png)

对于配置为UDP协议的event，SOME/IP支持在到达单播使用上限时自动在单播和组播之间切换；切换逻辑通过MULTICAST_THRESHOLD来控制，具体的切换逻辑用伪代码表示为：
